...
alter table tasks add column if not exists winner_team_id uuid references teams(id) on delete set null;
...
--
-- measurement_threshold and measurement_cap went the same way. They were
-- validated, snapshotted onto every submission and selected by eight routes,
-- and never once read by the scoring code.
--
