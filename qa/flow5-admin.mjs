...
try {
  ...
  const ghost = await call("/api/admin/tasks", {
    method: "PATCH",
    body: JSON.stringify({ id: "00000000-0000-0000-0000-000000000000", title: "__qa missing task" }),
  });
  check("an edit on a task that does not exist is refused, never a silent ok",
  ...
  const cut = await call(`/api/admin/tasks?id=${newTaskId}`, { method: "DELETE" });
  const { data: cutTask } = await admin.from("tasks").select("active").eq("id", newTaskId).single();
  const { data: keptSub } = await admin.from("submissions").select("id,object_name").eq("id", sub).single();
  check("removing a task cuts it without deleting its evidence",
    cut.status === 200 && cutTask?.active === false && keptSub?.id === sub && Boolean(keptSub?.object_name));
  await call("/api/admin/tasks", { method: "PATCH", body: JSON.stringify({ id: newTaskId, active: true }) });
